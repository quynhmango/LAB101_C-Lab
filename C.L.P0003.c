...
//Initialize the FileName variable to write the input value
void WriteFile(float d, char FileName[]){
	FILE* file=fopen(FileName,"w");//create a new file to write
    int n = fprintf(file,"%.2f",d);//write to file
	fclose(file);
}
//Read file
float ReadFile(char FileName[]){
	...
	if (file == NULL){
		...
		d=10.0;	//check money for players
	}else {
		int tem = fscanf(file,"%f",&d);//read to file
		if (d<0.25 || tem !=1) d=10.0;//money <0.25-> not enough to play or tem !=1: no data
		printf("File exist. Value is : %.2f",d);
		fclose(file);
	}
	return d;//return money read in file
}
...
float play(){
	...
	return d;//money after playing
}

int main(){
	int i;
	//Z:/SlotMachine.txt
	float e = ReadFile("Z:/SlotMachine.txt");
	do{
		printf("\nYou have %.2f\n",e);
